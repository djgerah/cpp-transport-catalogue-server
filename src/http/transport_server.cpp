...
void ts::TransportServer::PatchBus()
{
    listeners_.bus_listener.support(web::http::methods::PATCH, [&](web::http::http_request request) mutable {
        request.extract_json().then([request = std::move(request), this](pplx::task<web::json::value> task) mutable {
            try
            {
...
{
    listeners_.load_listener.support(web::http::methods::POST, [&](web::http::http_request req) mutable {
        req.extract_json().then([req = std::move(req), this](pplx::task<web::json::value> task) mutable {
            try
            {
                auto value = task.get();
                ...
                data_.json_reader = std::make_unique<json_reader::JsonReader>(document);
...
void ts::TransportServer::GetMap()
{
    listeners_.map_listener.support(web::http::methods::GET, [this](web::http::http_request request) mutable {
        try
        {
            ...
            auto svg_doc = data_.renderer->GetSVG(data_.catalogue.GetAllBuses());
...
{
    listeners_.stop_listener.support(web::http::methods::PUT, [&](web::http::http_request req) mutable {
        req.extract_json().then([req = std::move(req), this](pplx::task<web::json::value> task) mutable {
            try
            {
